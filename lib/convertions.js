...
function grammarToRegex(grammarDefinition) {
    ...
    const endState = grammarDefinition[grammarDefinition.length - 1].split(" -> ")[0];
...
function grammarToAutomata(grammarDefinition) {
    ...
    // Helper function to recursively build automata
    ...
    buildAutomata(grammarDefinition[0].split(" -> ")[0]);

    for (const transition of transitions) {
        const pointsToUndefined = transition.target === undefined;
        const emptyTransition = transition.symbol === "λ";
        if (pointsToUndefined && emptyTransition) {
            finalStates.add(transition.origin);
        }
    }
    alphabet.delete("λ");
    transitions = transitions.filter((transition) => transition.target !== undefined);

    return {
        alphabet: Array.from(alphabet),
        states: Array.from(states),
        transitions,
        initialState: grammarDefinition[0].split(" -> ")[0],
