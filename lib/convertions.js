...
const transitionEndsWithTerminalRegex = /^.*[a-z0-9]$/;
const grammarStateRegex = /(?=[A-Z])/;
 ...
 * @param {Array<string>} states
 */
function getLastPossibleState(states) {
  for (let i = 90; i >= 65; i--) {
    const char = String.fromCharCode(i);
    if (!states.includes(char)) {
      return char;
    }
  }

  throw Error("Not enough states to translate this grammar");
}
...
function grammarToAutomata(grammarDefinition) {
  ...
  function buildAutomata(state) {
    ...
    for (let transition of stateTransitions) {
      states.add(state);

      const transitionEndsWithTerminal = transition.match(transitionEndsWithTerminalRegex);
      if (transitionEndsWithTerminal) {
        const finalState = getLastPossibleState(Object.keys(transitionMap));
        transition += finalState;
        transitionMap[finalState] = ["λ"];
      }
      ...
      if (transition === "λ") {
        ...
        let [sequence, nextState] = transition.split(grammarStateRegex) 
        let char = sequence[0];
        if (sequence.length > 1) {
          const rest = sequence.slice(1);
          const generatedState = getLastPossibleState(Object.keys(transitionMap));
          transitionMap[generatedState] = [rest + nextState];
          nextState = generatedState;
        }
